...
EXCLUDED_CHECK_ITEMS_NORMALIZED: Set[str] = {
    EXCLUDED_KEYWORDS = set()  # キーワード除外が必要な場合はここに追加
    "ppt画像化",
    "ppt画像化",
    "印刷範囲外記載チェック[課題管理表]",
}
...
def is_excluded_check(check_id: object = "", check_item: object = "") -> bool:
    ...
    USER_FORCE_EXCLUDED = {
        "DE01", "DE03", "V1", "K01XY", "K01WBS", "K01NAME", "G02", "G04"
    }
    if cid in USER_FORCE_EXCLUDED:
        return True
...
def check_excel(file_path: Path, results: List[CheckResult]) -> None:
    ...
    for ws in wb.worksheets:
        try:
            ...
            orientation_str = str(orientation).lower() if orientation else 'portrait'
            is_vertical = orientation_str == 'portrait'
            pm = getattr(ws, 'page_margins', None)
            if pm:
                left = float(getattr(pm, 'left', 0)) * MM_PER_INCH
                right = float(getattr(pm, 'right', 0)) * MM_PER_INCH
                top = float(getattr(pm, 'top', 0)) * MM_PER_INCH
                bottom = float(getattr(pm, 'bottom', 0)) * MM_PER_INCH
                if is_vertical:
                    ok = (left >= 30 and right >= 20 and top >= 20 and bottom >= 20)
                    judge = "〇" if ok else "×"
                    margin_judgments.append(judge)
                    margin_details.append(f"[{ws.title}] 縦（Portrait）: 左={left:.1f}mm, 右={right:.1f}mm, 上={top:.1f}mm, 下={bottom:.1f}mm → {judge}")
                else:
                    ok = (top >= 30 and bottom >= 20 and left >= 20 and right >= 20)
                    judge = "〇" if ok else "×"
                    margin_judgments.append(judge)
                    margin_details.append(f"[{ws.title}] 横（Landscape）: 左={left:.1f}mm, 右={right:.1f}mm, 上={top:.1f}mm, 下={bottom:.1f}mm → {judge}")
            else:
                margin_details.append(f"[{ws.title}] 余白情報取得不可（縦横不明） → ×")
                margin_judgments.append("×")
        except Exception as e:
            margin_details.append(f"[{ws.title}] 余白取得エラー: {e}（縦横不明） → ×")
            margin_judgments.append("×")
    if margin_judgments and all(j == "〇" for j in margin_judgments):
        status = "PASS"
    elif margin_judgments and any(j == "〇" for j in margin_judgments):
        status = "WARN"
    else:
        status = "FAIL"
    detail = " / ".join(margin_details)
    add_result(results, file_path, "Excel", "C3", COMMON_MARGIN_RULE_TEXT, status, detail, "余白設定を確認・修正してください。")
...
def check_word(file_path: Path, results: List[CheckResult], cover_keyword: Optional[str]) -> None:
    ...
    for idx, section in enumerate(doc.sections):
        try:
            ...
            is_vertical = (getattr(orientation, 'value', orientation) == 0)
            left = float(getattr(section, 'left_margin', 0)) * MM_PER_INCH if getattr(section, 'left_margin', None) is not None else 0
            right = float(getattr(section, 'right_margin', 0)) * MM_PER_INCH if getattr(section, 'right_margin', None) is not None else 0
            top = float(getattr(section, 'top_margin', 0)) * MM_PER_INCH if getattr(section, 'top_margin', None) is not None else 0
            bottom = float(getattr(section, 'bottom_margin', 0)) * MM_PER_INCH if getattr(section, 'bottom_margin', None) is not None else 0
            if is_vertical:
                ok = (left >= 30 and right >= 20 and top >= 20 and bottom >= 20)
                judge = "〇" if ok else "×"
                margin_judgments.append(judge)
                margin_details.append(f"[Section{idx+1}] 縦（Portrait）: 左={left:.1f}mm, 右={right:.1f}mm, 上={top:.1f}mm, 下={bottom:.1f}mm → {judge}")
            else:
                ok = (top >= 30 and bottom >= 20 and left >= 20 and right >= 20)
                judge = "〇" if ok else "×"
                margin_judgments.append(judge)
                margin_details.append(f"[Section{idx+1}] 横（Landscape）: 左={left:.1f}mm, 右={right:.1f}mm, 上={top:.1f}mm, 下={bottom:.1f}mm → {judge}")
        except Exception as e:
    ...
    if margin_judgments and all(j == "〇" for j in margin_judgments):
        ...
        status = "FAIL"
    detail = " / ".join(margin_details)
    add_result(results, file_path, "Word", "C3", COMMON_MARGIN_RULE_TEXT, status, detail, "余白設定を確認・修正してください。")
